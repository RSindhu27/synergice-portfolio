  ...
  return (
    <View style={s.root}>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        ...
        >
          {/* Card */}
          <Animated.View style={[s.card, isWide && s.cardWide, { transform: [{ translateY: cardY }], opacity: cardOp }]}>
            ...
            <View style={s.form}>
              <View style={s.fieldWrap}>
                ...
                <View style={[s.inputRow, uFocus && s.inputFocused, !!error && !username.trim() && s.inputError]}>
                  <User size={14} color={uFocus ? COLORS.primaryLight : 'rgba(255,255,255,0.3)'} />
                  <TextInput
                    style={s.input}
                    value={username}
              ...
              <View style={s.fieldWrap}>
                ...
                <View style={[s.inputRow, pFocus && s.inputFocused, !!error && !password.trim() && s.inputError]}>
                  <Lock size={14} color={pFocus ? COLORS.primaryLight : 'rgba(255,255,255,0.3)'} />
                  <TextInput
          ...
          {isWide && (
            <View style={s.sidePanel}>
              <Text style={s.sideHeadline}>{'Unified pharma\nintelligence.'}</Text>
              <Text style={s.sideSub}>One platform to manage your entire portfolio, pipeline, and partnerships.</Text>
              <View style={s.featureList}>
                {[
                  { label: 'Portfolio Intelligence', sub: 'Real-time product analytics' },
                  { label: 'Revenue Insights', sub: 'Multi-region financial tracking' },
                  { label: 'Pipeline Visibility', sub: 'End-to-end deal monitoring' },
                  { label: 'Competitive Analysis', sub: 'Market positioning data' },
                ].map((f, i) => (
              ...
              <View style={s.statRow}>
                {[{ n: '140+', l: 'Products' }, { n: '28', l: 'Markets' }, { n: '94%', l: 'Uptime' }].map((st, i) => (
...
const s = StyleSheet.create({
  ...
  scrollWide: { flexDirection: 'row', justifyContent: 'center', alignItems: 'center' },
  card: {
  ...
  inputFocused: { borderColor: COLORS.primaryLight, backgroundColor: 'rgba(157,192,139,0.09)' },
  ...
  sidePanel: { flex: 1, maxWidth: 400, justifyContent: 'center', paddingLeft: 52, paddingRight: 20, paddingVertical: 40 },
  sideHeadline: { fontSize: 38, fontFamily: 'Poppins-Bold', color: '#fff', letterSpacing: -1, lineHeight: 46 },
  sideSub: { fontSize: 13, fontFamily: 'Poppins-Regular', color: 'rgba(255,255,255,0.45)', marginTop: 12, lineHeight: 21 },
  featureList: { marginTop: 32, gap: 18 },
  featureRow: { flexDirection: 'row', alignItems: 'flex-start', gap: 13 },
  featureDot: { width: 7, height: 7, borderRadius: 4, backgroundColor: COLORS.primaryLight, marginTop: 5 },
  featureLabel: { fontSize: 13, fontFamily: 'Poppins-SemiBold', color: '#fff' },
  featureSub: { fontSize: 11, fontFamily: 'Poppins-Regular', color: 'rgba(255,255,255,0.38)', marginTop: 1 },
  statRow: { flexDirection: 'row', gap: 10, marginTop: 36 },
  statCard: { flex: 1, backgroundColor: 'rgba(255,255,255,0.06)', borderRadius: 11, borderWidth: 1, borderColor: 'rgba(255,255,255,0.09)', padding: 13, alignItems: 'center' },
  statNum: { fontSize: 20, fontFamily: 'Poppins-Bold', color: COLORS.primaryLight },
  statLabel: { fontSize: 9, fontFamily: 'Poppins-Regular', color: 'rgba(255,255,255,0.35)', marginTop: 2, textTransform: 'uppercase', letterSpacing: 0.6 },
});
